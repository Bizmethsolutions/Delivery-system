...
export default class CustomerOrdersComponent extends PureComponent {
  ...
  componentDidMount = async()=> {
    ...
    let pickupdateCal = _.get(orderDataForEdit, 'pickupdate', '') !== '' ? (new Date(getDate(_.get(orderDataForEdit, 'pickupdate', '')))): ""
    ...
    if (suggest) {
      ...
      try {
        ...
        if(value && value.type === "success") {
          ...
          this.setState({ addressErr: false, displayadd: geoDetails.address })
        }
      } catch(e) {
  ...
  sendOrder=async()=> {
    ...
    if(Object.keys(err).length === 0) {
      ...
      if(pickupdate && pickupdate !== '' && _.get(orderDataForEdit, 'type', '') == "Removal") {
      orderData.pickupdate = pickupdate
      }
      if(pickupday && pickupday !== '' && _.get(orderDataForEdit, 'type', '') == "Removal") {
      ...
      if(parseFloat(this.state.totalamount) > 0) {
  ...
  confirmReject = async() => {
    let orderData  = this.props.orderDataForEdit
  ...
  updateCustomer = async() => {
    ...
    if (_.get(value, 'customers', []).length > 0) {
      let idx = _.get(value, 'customers', []).findIndex(obj => obj.id === _.get(this.props, 'selectedCustomer.id', ''))
        ...
        >
          <div className="react-modal-dialog react-modal-dialog-724 react-modal-dialog-centered">
            <div className="react-modal-header">
              <h5 className="react-modal-title d-flex align-items-center">Review Order - {_.get(orderDataForEdit, 'orderid', '')} <span className={_.get(orderDataForEdit, 'type', 'Delivery') === "Delivery" ? "greenlable unapproved-status pending" : "greenlable unapproved-status exchange"}>
              { containerName === "1/2 Yard" && _.get(orderDataForEdit, 'type', '') == "Exchange" ? "Mini Action" : _.get(orderDataForEdit, 'type', 'Delivery')}</span></h5>
              <h6 className="react-modal-title-sub"><span>Created On:</span> {createdInfo} <span className="pipeline">|</span> <span className="clearfixmob"></span> <span>By: </span>
            ...
            <div className="react-modal-body">
              ...
              { this.props.selectedCustomer && !this.props.selectedCustomer.isHomeCustomer ?
                <div className="row">
                  <div className="col-md-12">
                    <div className="form-group material-textfield">
                      <CurrencyInput
              ...
              {this.props.selectedCustomer && this.props.selectedCustomer.isHomeCustomer && _.get(orderDataForEdit, 'type', 'Delivery') !== 'Removal' ?
                <div>
                    <div className="mt-3">
